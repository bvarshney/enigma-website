...
const JobCard = ({ benefits, link, img, title, text, status }) => {
    ...
    return (
        <div className="w-full h-full fadeup">
            <Link href={jobPathBySlug(link)} className={`relative shadow-lg px-7 py-10 space-y-10 duration-300 rounded-2xl dark:bg-white2 hover:bg-white group grid grid-rows-[auto auto auto auto] h-full tablet:space-y-8 ${status === 'Closed' ? 'pointer-events-none' : ''}`}>
                ...
                <div className="mb-2">
                    <div className="flex items-center gap-4 h-full">
                        <div className="flex flex-col h-full justify-between pb-1">
                            {["benefit-1.svg", "benefit-2.svg", "benefit-3.svg", "benefit-4.svg"].map((item, index) => (
                                <Image key={index} className="w-6 h-6" src={`/assets/career/${item}`} alt="Benefits Image" width={15} height={15} />
                            ))}
                        </div>
                        <ul className="space-y-4 text-lg text-black2" dangerouslySetInnerHTML={{ __html: benefits }} />
                    </div>
                </div>
